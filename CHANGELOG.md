...
### Added
- **UI**: игровой скин виджета, управление окном игры, персист и рестайл окна Сравнение (d5bf96b)
  - **Игровой скин (T068)**: виджет и окно «Сравнение» оформлены в стиле вкладки STATUS игры — словарь `Themes/GameWidgetStyles.xaml` (бордовый заголовок, бежево-ореховая панель, золото/кремовый текст), бандл OFL-шрифта **Baloo 2** (аналог CookieRun — CookieRun запрещён лицензией в game-related ПО), переиспользуемый `OutlinedTextBlock` (тёмная окантовка), borderless title bar (перетаскивание через `WM_NCLBUTTONDOWN`, кнопка закрытия), бордовая полоса прогресса. Единый игровой скин (ADR-026), без адаптива Light/Dark.
  - **Управление окном игры**: `IGameWindowController`/`GameWindowController` (`TBHStats.Capture`) — кнопка уводит окно игры за пределы экрана (`SetWindowPos`) и возвращает; окно остаётся видимым для DWM → WGC-захват и сбор статистики не прерываются (сворачивание/скрытие их останавливают). Чекбокс «поверх всех окон» с персистом (`WidgetSettings.AlwaysOnTop`). Дочерние окна (Сравнение/Калибровка) закрываются вместе с виджетом.
  - **Персист геометрии окон**: обобщённый `WindowPlacement` (Core-модель + таблица, миграция `20260602174938_AddWindowPlacement`, репозиторий upsert по ключу). Окно «Сравнение» сохраняет позицию/размер между запусками (ключ «compare»); рестайл окна под скин виджета, светлые заголовки таблицы.
  - **Перевод UI на английский**: весь видимый текст виджета и окна сравнения (`Exp`, `Exp/h`, `Gold/h`, `Runs`, …).
  - **Фикс среднего золота/опыта за забег в «Сравнении»**: `StageAggregateRepository.CopyScalarFields` не копировал поля `AvgGoldGained/AvgXpGained/RecentAvg*` при обновлении агрегата (обнулялись), а существующие агрегаты не пересчитывались после миграции `AddAvgGained`. Добавлены копирование полей и идемпотентный self-heal бэкфилл (`DatabaseInitializer.BackfillStaleAggregatesAsync` на старте приложения).
  - Документация: `ARCHITECTURE.md` §6/§7, `DECISIONS.md` ADR-026, `data-model.md`. Сборка 0/0; тесты Data 97/97.

## [0.1.12] - 2026-06-02

### Added
- **Stats**: сегментная запись забегов, золото/опыт за этап в виджете и доработки таблицы Сравнение (378d3e4)
  - **Сегментная запись забега** (`StatsOrchestrator`/`RunRecorder`, ADR-025): запись `StageRun` переведена на границу сегмента этапа (сброс прогресса → босс → сброс) — тот же алгоритм, что вычисляет время предыдущего этапа. Источник золота/опыта/времени/сундуков единый; `IStageCompletionDetector` запись больше не управляет, `RunRecorder` стал тонким сервисом персиста (`PersistSegmentRunAsync`). Накопители сегмента (`_segmentStartGold`/`_segmentXpAccum` с компенсацией level-up/`_segmentChestAccum`) в оркестраторе.
  - **Гейтинг распознавания этапа**: забег пишется только при распознанном из свежего `nextLocation` этапе (`_segmentStageId`). На первых проходах и этапах с боссом акта (где `nextLocation` не отображается) забег не сохраняется, а в виджете в строке «Этап» — «—». Антифликер-стабилизатор `nextLocation` сохранён для транзиентного OCR-шума.
  - **Золото/опыт за этап в виджете**: в строках «Золото» и «Опыт» в скобках — значение за предыдущий пройденный этап (`LiveStatsSnapshot.LastCompletedStageGold/Xp`, `BuildGoldText`/`BuildXpCurrentText`) + поясняющие tooltips. Кнопка «Графики» убрана из виджета (окно `ChartsHostWindow` оставлено в коде).
  - **Ретенция 10 забегов**: на этап физически хранится не более 10 последних забегов — `RunRepository.PruneOldRunsAsync` (двухшаговый bulk-DELETE дочерних `StageRunChest` → `StageRun`) вызывается после записи и перед пересчётом агрегата; схема БД не менялась.
  - **Таблица «Сравнение»**: добавлены колонки «Золото»/«Опыт» (среднее абсолютное за забег — новые поля `StageAggregate.AvgGoldGained/AvgXpGained` + Recent, миграция `20260602131050_AddAvgGainedToStageAggregate`); номер этапа отделён от сложности (колонка сложности скрыта — сложность пока всегда Normal); убрана колонка «Сила» (данные уровня/урона сохраняются); убран no-op переключатель «Свежее окно/Вся история»; добавлено автообновление при появлении новых забегов (`RunRecorder.RunsChanged` → `CompareViewModel.StartAutoRefresh` с анти-реентранси и отпиской на `Unloaded`); зарезервировано место под полосу прокрутки.
  - Документация: `ARCHITECTURE.md` §6/§7/§9/§11, `DECISIONS.md` ADR-025. Сборка 0/0; тесты Core 298/298, Capture 115/115, Data 89/89.
...
### Added
- **Capture**: визуальная детекция прогресса этапа по цвету заливки (stageProgress) (fb5fcbe)
  - **Capture/Progress** (новое): `IStageProgressReader`/`StageProgressReader` (ADR-024) — поколоночный цветовой анализ ROI прогрессбара MainZone: фиолетовая заливка = прохождение пути, синяя = бой с боссом этапа. OCR неприменим (бар графический). `StageProgressReading(Progress, BossPresent)` + `StageProgressDiagnostic` (счётчики колонок + образец цвета для калибровки).
  - **Алгоритм** (доведён калибровкой на живой игре): высото-устойчивый абсолютный счёт колонок; относительная цветовая модель (инвариант к насыщенности — живой фиолетовый менее синий, чем на JPEG-фикстурах); нормировка по треку `фиол/(фиол+тёмн)`; ремап «мёртвых зон» бара `[0.12..0.84] → [0..0.95]` (стрелка-маркер + десатурированный край). Босс (синий) = `BossPresent=true`, прогресс 95% (этап не пройден, пока босс жив).
  - **Capture/FieldExtractor**: `stageProgress` включён вместо `null`-заглушек → `RawObservation.StageProgress`/`BossPresent` реально заполняются → питают `StageCompletionDetector` (сегментация забегов, ADR-012). Флаг `StageProgressDetectionEnabled`.
  - **App**: предпросмотр ROI `stageProgress` в калибровке через визуальный детектор (`DiagnoseAsync`), а не OCR; показ «Прогресс: N% · босс» + диагностика на живом кадре.
  - Верификация: 4 фикстуры `progress_*.jpg` (begin 0% / half 52% / full 95% / boss 95%·BossPresent); живая калибровка пусто→~0%, середина→~49%, полный→~95%, босс→95%, сброс после босса. Сборка Capture 0/0; тесты Capture 114/114.
