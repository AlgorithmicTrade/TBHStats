...
> Дата актуализации: **2026-06-03** (Иконка приложения: `Assets/AppIcon.ico` извлечена из оригинального `TaskBarHero.exe`; `<ApplicationIcon>` в csproj даёт иконку `.exe`, `AppWindow.SetIcon` — иконку окон/таскбара (Widget/Compare/Calibration/Charts); ARCHITECTURE.md §14. Ранее в тот же день: Compare: gold/h·exp/h теперь **взвешены по времени** (Σgained/Σdur·3600 = средн.добыча/этап ÷ средн.время/этап, сглаживает всплески, ADR-027) — формула едина для отображения и ранжирования; добавлена колонка «Time» (среднее время прохождения этапа, `AvgDurationSeconds`/`RecentAvgDurationSeconds`); self-disabling backfill пересчёта существующих агрегатов в `DatabaseInitializer`; **виджет не затронут** (живые темпы считаются независимо); ARCHITECTURE.md §7 / data-model.md / DECISIONS.md ADR-027 обновлены. Ранее: T068 Phase 2 — игровой скин виджета (палитра STATUS TBH, `GameWidgetStyles.xaml`, Pixelify Sans), заголовок-плашка «TBHStats», пергаментная панель статов; чекбокс «Поверх окон» FR-015 (AlwaysOnTop восстановлен из WidgetSettings); кнопка «Скрыть/Вернуть игру» через `IGameWindowController`; DI-регистрация; ADR-026; ARCHITECTURE.md §7 / DECISIONS.md обновлены. Ранее: T064 / ADR-025: сегментно-управляемая запись забегов — триггер `StageRun` в `StatsOrchestrator` по границе сегмента (падение прогресса после босса); gold/xp/сундуки по сегменту; гейтинг записи по распознанному из свежего `nextLocation` этапу (иначе «—» в строке «Этап»); `RunRecorder` → тонкий персист; ретенция ≤10 забегов на этап без миграции; `LastCompletedStageGold/Xp` в снапшоте (золото/опыт за прошлый этап в скобках); из виджета убрана кнопка «Графики»; в таблице сравнения этап/сложность разделены, колонки сложности и «Сила» скрыты; ARCHITECTURE.md §6/§7/§9/§11 + DECISIONS.md ADR-025. Ранее: FK-фикс HeroClasses: `GameMechanicsConfig.CreateDefault()` теперь содержит дефолтный класс `Id=1, Key="unknown"` — FK-якорь для `StageRun`; нераспознанный класс героя не делает забег `IsPartial=true`; §12 + §6 ARCHITECTURE.md. Ранее T063 Phase 2 fix: сегментный таймер синхронизирован с `StageProgress`; `LastCompletedStageSeconds` в `LiveStatsSnapshot`; `BuildStageElapsedText` в `LiveStatsViewModel`; ToolTip в виджете — §9 ARCHITECTURE.md; ранее T063 Phase 3: резолв `StageId` в `StatsOrchestrator` через `GameMechanicsConfig.ResolveStageId` (config-driven, ADR-009); сброс `RunRecorder` при `NotFound` с guard-флагом; тест `GameMechanicsConfigResolveStageIdTests` — §9 ARCHITECTURE.md; ранее T063 Phase 2: сегментный таймер этапа в `StatsOrchestrator`, проброс `StageProgress`/`BossPresent`/`StageElapsedSeconds` в `LiveStatsSnapshot`, строки «Прогресс этапа» + ProgressBar и «Время этапа» в виджете — §7/§9 ARCHITECTURE.md; ранее: золото — расходуемый баланс: монотонность не гейтит надёжность сэмпла, ADR-020 + spec/research/data-model; живой темп по короткому скользящему окну 90 с `LiveRateWindowSeconds`, §9 ARCHITECTURE.md; ранее: мульти-панельный UI — снятие tab-gating + апскейл мелких кропов OCR + порог 0.02 + перезагрузка ROI каждый кадр, ADR-019; мульти-позиционные ROI сундуков по числу типов + детекция раскладки, ADR-018; объединённая ROI опыта `xpPair` с парсингом по `/`; ранее T048: конфигурация поставки MSIX/unpackaged; T047: обработка ошибок и логирование)
