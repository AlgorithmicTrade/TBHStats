...
- **Кнопка «Графики» из виджета убрана (T064):** функционал графиков временно отключён — окно `ChartsHostWindow`/`ChartsView`/`ChartsViewModel` остаётся в коде (см. ниже), но точки входа из виджета нет. В виджете остаются только кнопки «Сравнение» и «Калибровка».
- **Keyboard accelerators (T053 A11y)**: Alt+C — сравнение, Alt+K — калибровка (Alt+G «графики» снят вместе с кнопкой). Все кнопки имеют `AutomationProperties.Name` и `AutomationProperties.AutomationId`.
