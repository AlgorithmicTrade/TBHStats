...
Строки виджета живой статистики (T028, T063 Phase 2):
- **Row 9 — «Прогресс этапа»**: `StackPanel`, внутри 2-колоночный `Grid` (подпись + `StageProgressText`, значение «49 %» / «Босс» / «—») + `ProgressBar` (`Value="{Binding StageProgress}"`, диапазон [0..1]). Биндинг из `LiveStatsViewModel.StageProgressText` / `StageProgress`.
- **Row 10 — «Время этапа»**: 2-колоночный `Grid` (подпись + `StageElapsedText`); формат «2м 05с» / «1ч 02м 03с» / «12с» / «—». Биндинг из `LiveStatsViewModel.StageElapsedText`.
- **Row 11 — статус + время обновления** (ранее Row 9, перенумеровано в T063 Phase 2).
