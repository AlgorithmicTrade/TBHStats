...
- **«Hero»**: класс + уровень героя. Урон (DPS) более **не отображается** в виджете (убран из строки Hero; `HeroDamage` продолжает сохраняться в БД через `RunRecorder`, но `LiveStatsViewModel.HeroDamage`/`HeroDamageText` удалены).

Переключатель режима и блок статистики сессии:
- **Переключатель «Забег ⇄ Сессия»** (`SessionViewCheckBox`, `CheckBox` `GameCheckBoxStyle`, `AutomationId="SessionViewCheckBox"`): первая строка панели статов, биндинг `IsChecked="{Binding IsSessionView, Mode=TwoWay}"`. Меняет **видимый** блок (одновременно виден один): блок «Забег» (видимость через `BoolToInverseVisibilityConverter` — виден при `IsSessionView == false`) ↔ блок «Сессия» (`BoolToVisibilityConverter`). `IsSessionView` — чисто UI-состояние, не перезаписывается в `ApplySnapshot`.
- **Блок «Сессия»** (5 строк): «Session gold» (`SessionGoldText`), «Session exp» (`SessionXpText`), «Stages» (`SessionStagesText`), «Levels» (`SessionLevelsText`), «Uptime» (`SessionUptimeText`). Источник — 5 полей `LiveStatsSnapshot.Session*`, накапливаемых в `StatsOrchestrator` за сессию (с момента старта оркестратора, см. §9). Золото/опыт — все положительные приросты; этапы — завершённые по боссу; уровни — суммарный прирост уровня героя; аптайм тикает непрерывно (включая Waiting/NotFound). Обнуляется при перезапуске процесса (приложение single-window: закрытие виджета завершает процесс).
