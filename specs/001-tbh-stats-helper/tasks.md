...
- [ ] T010 Первичная миграция EF Core + bootstrap SQLite (`%LOCALAPPDATA%\TBHStats\tbhstats.db`, авто-применение миграций) + сидинг механик в `src/TBHStats.Data/Migrations/`
- [ ] T011 [P] Интерфейсы и скелеты репозиториев (`IRunRepository`, `IStageAggregateRepository`, `ISettingsRepository`) в `src/TBHStats.Data/Repositories/`
- [ ] T012 [P] `IGameWindowTracker` — поиск окна TBH по HWND, отслеживание move/resize, видимость в `src/TBHStats.Capture/WindowTracking/GameWindowTracker.cs`
...
- [ ] T014 [P] `IOcrReader` — обёртка Windows.Media.Ocr (`SoftwareBitmap`, confidence, bounding boxes) в `src/TBHStats.Capture/Ocr/OcrReader.cs`
- [ ] T015 [P] ROI-маппер — нормализованные доли клиентской области ↔ пиксели кадра в `src/TBHStats.Capture/Roi/RoiMapper.cs`
- [ ] T016 [P] Тесты парсера (реальные строки K/M/B, время, stage-id) в `tests/TBHStats.Core.Tests/ValueParserTests.cs`
- [ ] T017 [P] Тесты ROI-маппера (инвариантность к размеру/масштабу) в `tests/TBHStats.Capture.Tests/RoiMapperTests.cs`
- [ ] T018 [P] Тесты репозиториев на реальном временном SQLite (CRUD, миграции) в `tests/TBHStats.Data.Tests/RepositoriesTests.cs`
